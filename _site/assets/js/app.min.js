...
InfoBox.prototype = new google.maps.OverlayView(), InfoBox.prototype.createInfoBoxDiv_ = function() {
    ...
    if (this.div_) {
        ...
        this.div_.style.WebkitTransform = "translateZ(0)", "undefined" != typeof this.div_.style.opacity && "" !== this.div_.style.opacity && (this.div_.style.MsFilter = '"progid:DXImageTransform.Microsoft.Alpha(Opacity=' + 100 * this.div_.style.opacity + ')"', 
    ...
    t.extend({
        simpleWeather: function(i) {
            ...
            if ("" !== i.location) {
                ...
                if ("" === i.woeid) return i.error("Could not retrieve weather due to an invalid location."), 
                !1;
            ...
            return t.getJSON(encodeURI(n), function(t) {
                if (null !== t && null !== t.query && null !== t.query.results && "Yahoo! Weather Error" !== t.query.results.channel.description) {
                    ...
                    r.title = n.item.title, r.temp = n.item.condition.temp, r.code = n.item.condition.code, 
                    r.todayCode = n.item.forecast[0].code, r.currently = n.item.condition.text, r.high = n.item.forecast[0].high, 
                    r.low = n.item.forecast[0].low, r.text = n.item.forecast[0].text, r.humidity = n.atmosphere.humidity, 
                    r.pressure = n.atmosphere.pressure, r.rising = n.atmosphere.rising, r.visibility = n.atmosphere.visibility, 
                    r.sunrise = n.astronomy.sunrise, r.sunset = n.astronomy.sunset, r.description = n.item.description, 
                    r.city = n.location.city, r.country = n.location.country, r.region = n.location.region, 
                    r.updated = n.item.pubDate, r.link = n.item.link, r.units = {
                    ...
                    "3200" == n.item.condition.code ? (r.thumbnail = a, r.image = a) : (r.thumbnail = "https://s.yimg.com/zz/combo?a/i/us/nws/weather/gr/" + n.item.condition.code + "ds.png", 
                    ...
                    }, "3200" == n.item.forecast[m].code ? (o.thumbnail = a, o.image = a) : (o.thumbnail = "https://s.yimg.com/zz/combo?a/i/us/nws/weather/gr/" + n.item.forecast[m].code + "ds.png", 
    ...
    $.fn.swipeEvents = function() {
        return this.each(function() {
            function touchstart(event) {
                ...
                touches && touches.length && (startX = touches[0].pageX, startY = touches[0].pageY, 
                $this.bind("touchmove", touchmove)), event.preventDefault();
            }
            function touchmove(event) {
                var touches = event.originalEvent.touches;
                if (touches && touches.length) {
                    var deltaX = startX - touches[0].pageX, deltaY = startY - touches[0].pageY;
                    deltaX >= 50 && $this.trigger("swipeLeft"), deltaX <= -50 && $this.trigger("swipeRight"), 
                    deltaY >= 50 && $this.trigger("swipeUp"), deltaY <= -50 && $this.trigger("swipeDown"), 
                    (Math.abs(deltaX) >= 50 || Math.abs(deltaY) >= 50) && $this.unbind("touchmove", touchmove);
                }
        ...
        function init_scroll(event, delta) {
            ...
            return timeNow - lastAnimation < quietPeriod + settings.animationTime ? void event.preventDefault() : (deltaOfInterest < 0 ? el.moveDown() : el.moveUp(), 
            void (lastAnimation = timeNow));
        }
        var settings = $.extend({}, defaults, options), el = $(this), sections = $(settings.sectionContainer);
        if (total = sections.length, status = "off", topPos = 0, lastAnimation = 0, quietPeriod = 500, 
        paginationList = "", $.fn.transformPage = function(settings, pos, index) {
            return $.support.transition ? ($(this).css({
            ...
            }), void $(this).one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend", function(e) {
                "function" == typeof settings.afterMove && settings.afterMove(index);
            })) : void $(this).animate({
                top: pos + "%"
            }, 400);
        }, $.fn.moveDown = function() {
            var el = $(this);
            if (index = $(settings.sectionContainer + ".active").data("index"), current = $(settings.sectionContainer + "[data-index='" + index + "']"), 
            next = $(settings.sectionContainer + "[data-index='" + (index + 1) + "']"), next.length < 1) {
                if (1 != settings.loop) return;
                pos = 0, next = $(settings.sectionContainer + "[data-index='1']");
            } else pos = 100 * index * -1;
            if ("function" == typeof settings.beforeMove && settings.beforeMove(current.data("index")), 
            current.removeClass("active"), next.addClass("active"), 1 == settings.pagination && ($(".onepage-pagination li a[data-index='" + index + "']").removeClass("active"), 
            $(".onepage-pagination li a[data-index='" + next.data("index") + "']").addClass("active")), 
            $("body")[0].className = $("body")[0].className.replace(/\bviewing-page-\d.*?\b/g, ""), 
            $("body").addClass("viewing-page-" + next.data("index")), history.replaceState && 1 == settings.updateURL) {
            ...
            next = $(settings.sectionContainer + "[data-index='" + (index - 1) + "']"), next.length < 1) {
                if (1 != settings.loop) return;
                pos = 100 * (total - 1) * -1, next = $(settings.sectionContainer + "[data-index='" + total + "']");
            ...
            el.transformPage(settings, pos, index);
        }, el.addClass("onepage-wrapper").css("position", "relative"), $.each(sections, function(i) {
            $(this).css({
                position: "absolute",
                top: topPos + "%"
            }).addClass("section").attr("data-index", i + 1), topPos += 100, 1 == settings.pagination && (paginationList += "<li><a data-index='" + (i + 1) + "' href='#" + (i + 1) + "'></a></li>");
        }), el.swipeEvents().bind("swipeDown", function() {
            el.moveUp();
        }).bind("swipeUp", function() {
            el.moveDown();
        }), 1 == settings.pagination && ($("<ul class='onepage-pagination'>" + paginationList + "</ul>").prependTo("body"), 
        posTop = el.find(".onepage-pagination").height() / 2 * -1, el.find(".onepage-pagination").css("margin-top", posTop)), 
        "" != window.location.hash && "#1" != window.location.hash) {
            if (init_index = window.location.hash.replace("#", ""), $(settings.sectionContainer + "[data-index='" + init_index + "']").addClass("active"), 
            $("body").addClass("viewing-page-" + init_index), 1 == settings.pagination && $(".onepage-pagination li a[data-index='" + init_index + "']").addClass("active"), 
            next = $(settings.sectionContainer + "[data-index='" + init_index + "']"), next && (next.addClass("active"), 
            1 == settings.pagination && $(".onepage-pagination li a[data-index='" + init_index + "']").addClass("active"), 
            $("body")[0].className = $("body")[0].className.replace(/\bviewing-page-\d.*?\b/g, ""), 
            $("body").addClass("viewing-page-" + next.data("index")), history.replaceState && 1 == settings.updateURL)) {
                var href = window.location.href.substr(0, window.location.href.indexOf("#")) + "#" + init_index;
                history.pushState({}, document.title, href);
            }
            pos = 100 * (init_index - 1) * -1, el.transformPage(settings, pos);
        } else $(settings.sectionContainer + "[data-index='1']").addClass("active"), $("body").addClass("viewing-page-1"), 
        1 == settings.pagination && $(".onepage-pagination li a[data-index='1']").addClass("active");
        return $(".onepage-pagination li a").click(function() {
            var page_index = $(this).data("index");
            if ($(this).hasClass("active") || (current = $(settings.sectionContainer + ".active"), 
            next = $(settings.sectionContainer + "[data-index='" + page_index + "']"), next && (current.removeClass("active"), 
            next.addClass("active"), $(".onepage-pagination li a.active").removeClass("active"), 
            $(".onepage-pagination li a[data-index='" + page_index + "']").addClass("active"), 
            $("body")[0].className = $("body")[0].className.replace(/\bviewing-page-\d.*?\b/g, ""), 
            $("body").addClass("viewing-page-" + next.data("index"))), pos = 100 * (page_index - 1) * -1, 
            el.transformPage(settings, pos)), 0 == settings.updateURL) return !1;
        }), $("section, ul.onepage-pagination").bind("mousewheel DOMMouseScroll", function(event) {
            event.preventDefault();
            var delta = event.originalEvent.wheelDelta || -event.originalEvent.detail;
            $("body").hasClass("disabled-onepage-scroll") || init_scroll(event, delta);
        }), !1;
    };
    ...
    e.fn.cycle = function(i) {
        ...
        updateView: function(e) {
            ...
            if (t._initialized) {
                ...
                !e && (t.API.trigger("cycle-update-view-before", [ t, i, n ]), 0 > t.updateView) || (t.slideActiveClass && t.slides.removeClass(t.slideActiveClass).eq(t.currSlide).addClass(t.slideActiveClass), 
                e && t.hideNonActive && t.slides.filter(":not(." + t.slideActiveClass + ")").hide(), 
                t.API.trigger("cycle-update-view", [ t, i, n, e ]), t.API.trigger("cycle-update-view-after", [ t, i, n ]));
            }
        },
    ...
    function t(t, i) {
        ...
        void t.slides.each(function(s) {
            return e(this).data("cycle-hash") == n ? (i === !0 ? t.startingSlide = s : (t.nextSlide = s, 
            t.API.prepareTx(!0, !1)), !1) : void 0;
        }));
    }
